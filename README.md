...
websocet接続でturbowarpのサーバーに接続し変数として取得した後桁ごとに適切な処理を行います。具体的には
 - 観測点ID:1~小数点(.)までの値がそのままID

なお、これ以降の桁は小数点の位置を0とした場合の桁数です。
 ...
 - 各方向の過去10秒間の波形:27~最後までを使用している。詳しくは後。
...
つまり、例えばNS方向のの10秒前のデータを得たい場合は、27~31桁の数を使用します。  
これをxとしたときにx/10-1000をすることで、加速度値になります。  

EW方向の10秒前のデータを得たい場合はまず27~最後までの桁数を求める必要があります。  
それを3で割ることで一方向が占めている桁数が分かるので、その桁数を27に足した数の桁から+5文字目までをxとして以上の処理を行うことで求められます。
